...
// 지점 상세 정보를 제공한다.
final storeDetailProvider = FutureProvider.family<StoreSummary, String>((
  ref,
  storeId,
) async {
  final repository = ref.watch(storeRepositoryProvider);
...
final storeBreakdownProvider = FutureProvider.family<RatingBreakdown, String>((
  ref,
  storeId,
) async {
  final repository = ref.watch(storeRepositoryProvider);
...
// 상세항목 평가가 비슷한 지점을 제공한다.
final similarStoresProvider = FutureProvider.family<List<SimilarStore>, String>(
  (ref, storeId) async {
    final repository = ref.watch(storeRepositoryProvider);
    return repository.fetchSimilarStores(storeId);
  },
);
